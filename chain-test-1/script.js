...
const runNewman = (inputFile) => {
    newman.run({
        collection: require('./collection.json'), // Path to the Postman collection
        reporters: 'cli',
        ...
        if (data.item.name === 'Convert Draft to Active Record') {
            try {
                // Read the existing data file
                const fileData = await fs.readFile(inputFile, 'utf8');
                ...
                console.log(`"Log -> Updated request body for Convert Draft to Active Record: ${JSON.stringify(requestBody)}"`);

                // Add a delay after the Convert Draft to Active Record request
                await delay(10000); // 20 seconds delay
                console.log('Log -> Delay after Convert Draft to Active Record completed.');

            } catch (err) {
                console.error(`"Error -> Error processing Convert Draft to Active Record request: ${err}"`);
            }
        }

        // Handle Get One Record request
        if (data.item.name === 'Get One Record') {
            try {
                // Read the existing data file
                const fileData = await fs.readFile(inputFile, 'utf8');
                ...
                // Get the record ID and business ID from the file
                const recordId2 = jsonData[0].record_id;
                const businessId2 = jsonData[0].business_id;

                // Print the recordId and businessId for debugging
                console.log(`"Log -> Record ID for Get One Record: ${recordId2}"`);
                console.log(`"Log -> Business ID for Get One Record: ${businessId2}"`);

                // Replace the recordId and businessId in the request URL
                const updatedUrl2 = data.item.request.url.toString()
                    .replace(':businessId', businessId2)
                    .replace(':recordId', recordId2);

                data.item.request.url = updatedUrl2;

                // Print the updated URL for debugging
                console.log(`"Log -> Updated URL for Get One Record: ${updatedUrl2}"`);
            } catch (err) {
                console.error(`"Error -> Error processing Get One Record request: ${err}"`);
            }
        }
